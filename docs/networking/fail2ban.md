...
Fail2ban can parse Caddy's JSON access logs to ban IPs that hit too many 4xx errors. First, enable JSON logging in your Caddyfile:
...
Prevent accidentally locking yourself out by whitelisting your local subnet and Tailscale address range. Create `/etc/fail2ban/jail.d/local.conf`:
...
| Banned yourself | Unban with `sudo fail2ban-client set sshd unbanip YOUR_IP`; add your IP to `ignoreip` in `jail.d/local.conf` |
...
| Caddy jail never triggers | Confirm Caddy is writing JSON logs to the path in `logpath`; test the regex with `sudo fail2ban-regex /var/log/caddy/access.log /etc/fail2ban/filter.d/caddy.conf` |
