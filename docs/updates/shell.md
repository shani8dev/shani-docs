...
- **KDE Plasma:** **Konsole**, pre-installed, with the Saturn colour scheme and a translucent blurred background. It runs your login shell, so `chsh` applies there too. **FiraMono Nerd Font** is the system fixed-width font, shared by Konsole, Yakuake, Kate and every other KDE app (*System Settings → Text & Fonts → Fixed width*). **Yakuake**, a drop-down terminal, is also pre-installed: press `F12` to toggle it from anywhere.
