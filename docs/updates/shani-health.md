...
To automate that on a schedule, run it from a timer you own — Shanios does
not ship one. Note the `Unit=` line in the timer: without it systemd looks
for a *service* named after the timer, and the report never runs.

```ini
# /etc/systemd/system/shani-health-report.service
[Unit]
Description=Daily shani-health report
...
[Unit]
Description=Run the daily shani-health report

[Timer]
OnCalendar=daily
Persistent=true
Unit=shani-health-report.service
...
```bash
sudo systemctl enable --now shani-health-report.timer
```

The `%%` in the date format is not a typo: systemd treats `%` as its own
specifier escape, so a literal percent must be doubled inside a unit file.
