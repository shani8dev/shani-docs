...
1. **Check** — The per-user `shani-cassini-agent.timer` checks update and boot state after login and every 2 hours
2. **Notify** — The Shani Cassini agent sends a desktop notification; open **Updates & Rollback** to install an available update
...
- The Shani Cassini agent checks for a fallback or candidate boot after login and links to **Updates & Rollback** when attention is needed
